...
use crate::experiments::{
    load_experiment_config, RealGenerationMetrics, UnseenPocketExperiment,
...
pub fn run_generation_demo_from_config(
    ...
    let chemistry = report_to_metrics(
        ...
        "active heuristic chemistry-validity backend on modular generation demo",
    );
    let docking = report_to_metrics(
        HeuristicDockingEvaluator.evaluate_docking(&candidates),
        "active heuristic docking-oriented hook on modular generation demo",
    );
    let pocket = report_to_metrics(
        HeuristicPocketCompatibilityEvaluator.evaluate_pocket_compatibility(&candidates),
        "active heuristic pocket-compatibility hook on modular generation demo",
    );
